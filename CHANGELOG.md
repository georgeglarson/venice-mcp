...
### Fixed
- **Image generation saves to file**: Images are now saved to `~/venice-images/` and the file path is returned, avoiding MCP's "tool result too large" error in Claude Desktop

## [1.2.4] - 2025-12-05

### Fixed
- **Image generation display in Claude Desktop**: Return images using MCP's native `image` content type instead of base64 text, so Claude Desktop can render images directly
